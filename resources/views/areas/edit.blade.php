<x-app-layout>
    <div class="max-w-lg mx-auto mt-10 bg-white dark:bg-neutral-900 rounded-xl shadow p-8">
        <h2 class="text-2xl font-bold mb-6 text-blue-700 dark:text-blue-400">Editar Área</h2>
        <form method="POST" action="{{ route('areas.update', $area->id_area) }}" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                <label class="block font-semibold mb-1">ID Área</label>
                <input type="text" name="id_area" class="form-input w-full rounded-lg bg-gray-200 dark:bg-neutral-800" value="{{ $area->id_area }}" disabled>
            </div>
            <div>
                <label class="block font-semibold mb-1">Nombre</label>
                <input type="text" name="nombre" class="form-input w-full rounded-lg" required value="{{ old('nombre', $area->nombre) }}">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="superficie_ha" class="form-input w-full rounded-lg" required value="{{ old('superficie_ha', $area->superficie_ha) }}">
